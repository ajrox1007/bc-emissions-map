    ...
    for (const row of relevantProjects) {
      ...
      await libsql.execute({
        ...
        args: [
          ...
          row["PROJECT_CATEGORY_NAME"] ? String(row["PROJECT_CATEGORY_NAME"]) : null,
          row["PUBLIC_FUNDING_IND"] ? 1 : 0,
          row["PROVINCIAL_FUNDING"] ? 1 : 0,
          row["FEDERAL_FUNDING"] ? 1 : 0,
          row["MUNICIPAL_FUNDING"] ? 1 : 0,
          row["OTHER_PUBLIC_FUNDING"] ? 1 : 0,
          row["GREEN_BUILDING_IND"] ? 1 : 0,
          row["CLEAN_ENERGY_IND"] ? 1 : 0,
          row["INDIGENOUS_IND"] ? 1 : 0,
          row["STANDARDIZED_START_DATE"] ? String(row["STANDARDIZED_START_DATE"]) : null,
          row["STANDARDIZED_COMPLETION_DATE"] ? String(row["STANDARDIZED_COMPLETION_DATE"]) : null,
